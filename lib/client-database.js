// Client-side database using localStorage with fallback to shared files
class ClientDatabase {
  ...
  initializeStorage() {
    ...
    if (typeof window !== 'undefined' && window.localStorage) {
      ...
      // Server environment - initialize empty arrays
      console.log('Running in server environment, using shared data files');
    }
  }

  // Members
  getMembers() {
    // Try to read from shared data files first
    if (typeof window === 'undefined') {
      // Server environment - try to read from data files
      try {
        const fs = require('fs');
  ...
  getDashboardData() {
    ...
    const todayAttendance = attendance.filter(record => 
      ...
      return {
        ...
        memberId: member ? member.memberId : 'Unknown'
      };
    });
